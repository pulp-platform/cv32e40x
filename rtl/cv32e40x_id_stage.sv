...
(
  ...
  generate
    if (X_EXT) begin : x_ext
      ...
      logic xif_accepted_q, xif_rejected_q, xif_we_q, xif_exception_q, xif_dualwrite_q, xif_loadstore_q;
      ...
      // (TODO-is it fixed? I don't know: it should but I haven't done extensive testing (rfiorani)):XIF These may be missed if issue_valid retracts before ID goes to EX. Need to check for sticky accept as well
      assign xif_we        = (xif_issue_if.issue_valid && xif_issue_if.issue_ready && xif_issue_if.issue_resp.accept && xif_issue_if.issue_resp.writeback) || xif_we_q;      
      assign xif_exception = (xif_issue_if.issue_valid && xif_issue_if.issue_ready && xif_issue_if.issue_resp.accept && xif_issue_if.issue_resp.exc      ) || xif_exception_q;
      assign xif_dualwrite = (xif_issue_if.issue_valid && xif_issue_if.issue_ready && xif_issue_if.issue_resp.accept && xif_issue_if.issue_resp.dualwrite) || xif_dualwrite_q;
      assign xif_loadstore = (xif_issue_if.issue_valid && xif_issue_if.issue_ready && xif_issue_if.issue_resp.accept && xif_issue_if.issue_resp.loadstore) || xif_loadstore_q;
